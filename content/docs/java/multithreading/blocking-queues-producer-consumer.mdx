...
With `BlockingQueue`, if the queue is empty, the consumer simply sleeps internally until data is available.

Similarly, if the queue is full, the producer sleeps internally until space becomes available.
