...
Now when one thread changes `running`, other threads can observe the updated value correctly.
...
* `Atomic` classes -> lock-free thread-safe operations for simple updates
